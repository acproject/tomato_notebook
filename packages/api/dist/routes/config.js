...
config.put('/ai', async (c) => {
    try {
        ...
        if (body.apiBase !== undefined)
            setConfigValue('ai.apiBase', body.apiBase);
        if (body.port !== undefined)
            setConfigValue('ai.port', String(body.port));
        if (body.model !== undefined)
            setConfigValue('ai.model', body.model);
        if (body.apiKey !== undefined)
            setConfigValue('ai.apiKey', body.apiKey);
        return c.json({
            success: true,
            data: {
...
config.put('/miniMemory', async (c) => {
    try {
        ...
        if (body.host !== undefined)
            setConfigValue('miniMemory.host', body.host);
        if (body.port !== undefined)
            setConfigValue('miniMemory.port', String(body.port));
        if (body.enabled !== undefined)
            setConfigValue('miniMemory.enabled', String(body.enabled));
        if (body.password !== undefined)
            setConfigValue('miniMemory.password', body.password);
        return c.json({
            success: true,
            data: {
...
config.put('/server', async (c) => {
    try {
        const body = await c.req.json();
        if (body.port !== undefined)
            setConfigValue('server.port', String(body.port));
        if (body.host !== undefined)
            setConfigValue('server.host', body.host);
        return c.json({
            success: true,
            data: {

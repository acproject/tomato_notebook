...
function getAIConfig() {
    const rows = db.prepare('SELECT key, value FROM config WHERE key LIKE ?').all('ai.%');
    const config = {};
...
ai.post('/summarize/:id', async (c) => {
    ...
    try {
        const prompt = PROMPTS.summarize[length]
...
ai.post('/polish/:id', async (c) => {
    ...
    try {
        const prompt = PROMPTS.polish[style]
...
ai.post('/chat/:sessionId', async (c) => {
    ...
    try {
        ...
        let context = '';
        if (session.noteId) {
            const note = db.prepare('SELECT title, content FROM notes WHERE id = ?').get(session.noteId);
            if (note) {
                context = `当前笔记内容：\n标题：${note.title}\n内容：${note.content}\n\n`;
            }
        }

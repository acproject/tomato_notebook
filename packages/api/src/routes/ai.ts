...
ai.post('/summarize/:id', async (c) => {
  ...
  const response = await services.ai.summarizeNote(id, length as 'short' | 'medium' | 'long');
...
ai.post('/polish/:id', async (c) => {
  ...
  const response = await services.ai.polishNote(id, style as 'formal' | 'casual');
...
ai.post('/execute', async (c) => {
  ...
  const operationMap: Record<string, AIOperation> = {

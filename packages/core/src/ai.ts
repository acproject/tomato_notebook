import type { OllamaConfig, AIRequest, AIResponse, Message, AISession } from './types.js';
...
export class AIService {
  ...
  // 获取系统提示词
  private getSystemPrompt(operation: AIOperation): string {
    const systemPrompts: Record<AIOperation, string> = {
  ...
  // 总结笔记
  async summarizeNote(noteId: string, length: 'short' | 'medium' | 'long' = 'medium'): Promise<AIResponse> {
  ...
  // 润色笔记
  async polishNote(noteId: string, style: 'formal' | 'casual' = 'formal'): Promise<AIResponse> {
    const note = await this.noteService.getNote(noteId);
    if (!note) {
      return { success: false, error: 'Note not found' };
    }
  ...
  // 翻译笔记
  async translateNote(noteId: string, language: string): Promise<AIResponse> {
    const note = await this.noteService.getNote(noteId);
    if (!note) {
      return { success: false, error: 'Note not found' };
    }
  ...
  // 获取学习建议
  async getSuggestions(context?: string, noteId?: string): Promise<AIResponse> {
    let content = context || '';
  ...
  // 创建聊天会话
  async createChatSession(noteId?: string): Promise<AISession> {
  ...
  // 发送聊天消息
  async chat(sessionId: string, message: string): Promise<AIResponse> {

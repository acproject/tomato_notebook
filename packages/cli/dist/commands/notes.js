...
function printNotes(notes) {
    ...
    notes.forEach((note, index) => {
        ...
        console.log(`      ${chalk.gray(formatDate(note.updatedAt))} ${chalk.gray(note.tags.map((t) => `#${t}`).join(' '))}`);
    });
}
export function registerNoteCommands(program) {
    ...
    notes
        ...
        try {
            const response = await api.get(`/api/notes?filter=${options.filter}&limit=${options.limit}`);
            spinner.stop();
            if (response.success && response.data) {
    ...
    notes
        ...
        try {
            const response = await api.post(`/api/notes/${id}/tags`, { tags });
            if (response.success && response.data) {
                spinner.succeed(`标签已添加: ${response.data.tags.map((t) => chalk.blue(`#${t}`)).join(' ')}`);
            }
            else {
    ...
    notes
        ...
        try {
            const response = await api.get('/api/notes/stats/summary');
            spinner.stop();
            if (response.success && response.data) {

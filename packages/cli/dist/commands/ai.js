...
export function registerAICommands(program) {
    ...
    ai
        ...
        try {
            const response = await api.post(`/api/ai/summarize/${noteId}?length=${options.length}`);
            if (response.success && response.data) {
    ...
    ai
        ...
        try {
            const response = await api.post(`/api/ai/polish/${noteId}?style=${options.style}`);
            if (response.success && response.data) {
    ...
    ai
        ...
        .action(async (noteId, language) => {
        spinner.start(`正在翻译为${language}...`);
        try {
            const response = await api.post(`/api/ai/translate/${noteId}?language=${encodeURIComponent(language)}`);
            if (response.success && response.data) {
    ...
    ai
        ...
        try {
            ...
            const sessionResponse = await api.post('/api/ai/chat/session', options.note ? { noteId: options.note } : {});
            ...
            while (true) {
                ...
                if (!input.trim())
                    continue;
                spinner.start('思考中...');
                const chatResponse = await api.post(`/api/ai/chat/${sessionId}`, { message: input });
                spinner.stop();

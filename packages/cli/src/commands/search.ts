...
export function registerSearchCommands(program: Command): void {
  ...
  search
    ...
    .action(async (query: string, options: { 
      ...
      try {
        ...
        if (options.favorite) url += `&favorite=true`;
        if (options.aiGenerated) url += `&ai-generated=true`;
        ...
        if (response.success && response.data) {
          ...
          response.data.forEach((note, index) => {
            ...
            console.log(`      ${chalk.gray(new Date(note.updatedAt).toLocaleDateString('zh-CN'))} ${chalk.gray(note.tags.map((t: string) => `#${t}`).join(' '))}`);
          });
  ...
  search
    ...
    .action(async (query: string) => {
      try {
        const response = await api.get<APIResponse<Note[]>>(`/api/search/quick?q=${encodeURIComponent(query)}`);
  ...
  search
    ...
    .action(async (query: string) => {
      try {
        const response = await api.get<APIResponse<string[]>>(`/api/search/suggestions?q=${encodeURIComponent(query)}`);
